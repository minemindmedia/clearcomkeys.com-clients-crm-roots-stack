...
const seed = (app) => {
    app.hooks
        ...
        .hooks.on(`middleware.dev.options.publicPath`, () => app.hooks.filter(`build.output.publicPath`, `/`))
        ...
        .hooks.on(`middleware.hot.options.log`, app.logger.instance.scope('hot').info)
        .hooks.on(`middleware.hot.options.heartbeat`, 2000)
        .hooks.on(`dev.client.scripts`, new Set([
        app => src(`index.js?name=${app.name}&bud.overlay=${app.context.args.overlay}&bud.indicator=${app.context.args.indicator}&path=${app.hooks.filter('middleware.hot.options.path')}`),
